...
void printation( char m[MAX][MAX], int lin, int col, relampago_mcqueen carros[], int n_carros) {
  ...
  for (int i = 0; i < lin; i++)
    for(int j = 0; j < col; j++) {
      ...
      else if ( qtd_carros == 1 && carros[0].lin == i && carros[0].col == j ) printf( "\033[1;31;41m%c\033[0m", carros[index].direction);
      else if ( qtd_carros == 1) printf("\033[2;32;42m%c\033[0m", carros[index].direction);
